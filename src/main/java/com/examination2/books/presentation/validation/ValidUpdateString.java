...
/**
 * {@code @ValidUpdateString} アノテーションは、更新時の入力内容の妥当性を検証するために使用されます。
 * このアノテーションは、フィールドレベルで使用され、指定された入力内容が有効であるかどうかを検証します。
 */
@Documented
@Constraint(validatedBy = UpdateStringValidator.class)
@Target({FIELD})
@Retention(RUNTIME)
public @interface ValidUpdateString {
     ...
     */
    String message() default "Invalid update value";
